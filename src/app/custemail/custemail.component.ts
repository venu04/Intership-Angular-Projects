...
@Component({
  selector: 'app-custemail',
  templateUrl: './custemail.component.html',
  styleUrls: ['./custemail.component.css']
})
export class CustemailComponent implements OnInit {
  httpClient:any;   
  public output=[];
  ...
  loginUser()
  {
    ...
    if(this.name=="")
    {
      this.loader="normal";
      this.showError = true;
      this.router.navigate[("/custforgetpassword")];
    }
    else
    {
      ...
      localStorage.setItem('custemailotp',val.toString());
      var otp=localStorage.getItem('custemailotp');
      console.log('otp ',localStorage.getItem('custemailotp'));
      const  postData1 = {
        "tootp": otp,
        "tomail":this.name,
        "name":"Customer"
